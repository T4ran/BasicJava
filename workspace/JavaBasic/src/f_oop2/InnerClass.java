...
class Outer
{
	...
	static class StaticInner
	{
		...
		//int inner1 = instanceVar;		//인스턴스 변수는 객체생성을 해야해서 선언불가
		int inner2 = classVar;
	...
	void method()
	{
		...
		class LocalInner
		{
			...
			int inner1 = instanceVar;
			int inner2 = classVar;
			//int inner3 = localVar;		//final이 붙지 않은 지역변수는 사용 불가
	...
	void test1()
	{
		//인스턴스 클래스의 인스턴스 멤버에 접근하기 위해서 인스턴스화 해야한다.
		...
		//스태틱 클래스의 인스턴스 멤버에 접근하기 위해서 인스턴스화 해야한다.
		Outer.StaticInner si = new Outer.StaticInner();
		System.out.println(si.inner2);
	}
}
