...
class Exercise3_1 {
	public static void main(String[] args) {
		...
		6		int는 4바이트=32비트이며 쉬프트연산자는 데이터 타입의 범위를 벗어나면 데이터 타입의 범위로 들어올 때 까지 데이터 타입만큼 빼준다.
				따라서 저 식은 1 + x << 1과 같아진다.
...
(1) <== (numOfApples % sizeOfBucket != 0) ? 1 + numOfApples / sizeOfBucket : numOfApples / sizeOfBucket
...
(1) <== (int)(5.0f / 9.0f * (fahrenheit - 32) * 100.0f + 0.5f) / 100.0f
...
class Exercise3_8 {
	public static void main(String[] args) {
		byte a = 10;
		byte b = 20;
		byte c = a + b;		<= byte를 int로 변경
		
		char ch = 'A';
		ch = ch + 2;		<= ch + 2를 (char)(ch + 2)로 변경
		
		float f = 3 / 2;	<= 3 혹은 2를 3.0f 혹은 2.0f로 변경
		long l = 3000 * 3000 * 3000;	<= 3000을 3000L로 변경
		
		float f2 = 0.1f;
		double d = 0.1;
		
		boolean result = d==f2;		<== d를 (float)d로 변경
...
(1) <== ch >= 'a' && ch <='z' || ch >= 'A' && ch <= 'Z' || ch >= '0' && ch <= '9'
